...
# Install or update vim-language-server - João Vítor Carli Pereira
# LICENSE MIT

ASDF_NODE_VERSIONS=$(asdf shim-versions node | sed 's/[^ ]* //')
ASDF_NODE_PATH="$HOME/.asdf/installs/nodejs"
TMP_DIR="/tmp/nvim-langservers/vim-language-server/"
UPDATE_CMD="install -g vim-language-server"

for VERSION in $ASDF_NODE_VERSIONS; do
